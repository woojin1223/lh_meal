...
train_recipe <- training_set %>% 
  recipe(N_lunch ~ .) %>%
  step_rm(Date, Breakfast, Lunch, Dinner, N_dinner, Rate_lunch, Rate_dinner) %>%
...
rf_cv_results %>% show_best() # mtry=12, CV mae=70.6
...
bayes_cv_results %>% show_best() # mae=87.4
bayes_cv_results %>% select_best("mae") # mtry=12, min_n=14, tree_depth=11
...
train_recipe <- training_set %>% 
  recipe(N_dinner ~ .) %>%
  step_rm(Date, Breakfast, Lunch, Dinner, N_lunch, Rate_lunch, Rate_dinner) %>%
  step_scale(all_numeric(), -all_outcomes()) %>% 
  step_string2factor(all_nominal())
...
rf_cv_results %>% show_best() # mtry=12, CV mae=44.0
...
# Find best hyperparameter
bayes_cv_results %>% show_best() # mae=57.6
...
submission %>% fwrite("submission_0712_second.csv")
```