...
get_menu_encoding_dict <- function() {
    ...
    # Fish: '연어|장어|열기|고등어|삼치|꽁치|갈치|가자미|굴비|조기|생선|임연수|동태|명태|코다리|홍어|황태|참치|멸치'
    # Clam: "조개|골뱅이|전복|굴[^(소스)]"
    # Shrimp: "새우|랍스터|꽃게"
    ...
    dict[["Desert"]] <- "요구르트|[가-힣]+D|[가-힣]*음료|[가-힣]*호떡|식혜|청포도|바나나|복숭아|오렌지|수박|[가-힣]*토마토|[가-힣]+주스|[가-힣]*아이스티|[가-힣]+빵|[가-힣]*케익|요플레|[가-힣]*츄러스|[가-힣]*화채"
    dict[["Salad"]] <- "[가-힣]*샐러드"
    ...
    dict[["Beef"]] <- "장조림|바베큐|떡갈비|언양식|소고기|쇠|소불고기|쇠|쇠고기|우육|소갈비"
    dict[["Chicken"]] <- "후라이드|윙|닭|치킨"
    dict[["Duck"]] <- "오리"
    dict[["Processed_meat"]] <- "미트|소세지|너비아니|함박|완자"
    dict[["Fish"]] <- "연어|장어|열기|고등어|삼치|꽁치|갈치|가자미|굴비|조기|생선|임연수|동태|명태|코다리|홍어|황태|참치|멸치"
